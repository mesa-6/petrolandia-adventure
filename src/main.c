	...
	void DrawGameOverScreen(void) {
		// Array para armazenar as entradas do ranking
		RankEntry ranking[MAX_RANKING];
		int count = 0;
		
		// Abrir o arquivo de ranking e carregar os dados
		...
		// Ler até cinco entradas de ranking
		while (fscanf(file, "%29[^;];%d;%d\n", ranking[count].nome, &ranking[count].score, &ranking[count].banhistas) == 3 && count < MAX_RANKING) {
			count++;
		}
		fclose(file);
		...
		// Texto de Game Over
		DrawText("GAME OVER", 160, 20, 50, RED);
		
		// Exibir o ranking
		DrawText("TOP 5 MELHORES JOGADORES", 100, 100, 30, DARKBLUE);
		for (int i = 0; i < count; i++) {
			char rankLine[100];
			sprintf(rankLine, "%d. %s - Score: %d - Banhistas: %d", i + 1, ranking[i].nome, ranking[i].score, ranking[i].banhistas);
			DrawText(rankLine, 100, 150 + i * 30, 20, DARKGREEN);
		}

		// Opções de reiniciar ou ir ao menu
		DrawText("PRESSIONE ENTER PARA REINICIAR", 100, 300, 25, DARKBLUE);
		DrawText("PRESSIONE M PARA VOLTAR AO MENU", 100, 350, 25, DARKBLUE);
